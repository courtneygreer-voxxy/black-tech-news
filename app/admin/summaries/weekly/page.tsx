...
export default function WeeklySummariesAdmin() {
  ...
  const fetchSummaries = async () => {
    try {
      const response = await fetch('/api/admin/summaries/weekly');
  ...
  const generateNewSummary = async () => {
    ...
    setGenerating(true);
    try {
      const response = await fetch('/api/admin/summaries/weekly/generate', {
        method: 'POST',
      });
      ...
      if (data.success) {
        alert(`Summary generated! ${data.articleCount} articles included.`);
  ...
  const togglePublish = async (id: number, currentStatus: boolean) => {
    try {
      const response = await fetch(`/api/admin/summaries/weekly/${id}/publish`, {
        method: 'POST',
  ...
  const deleteSummary = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/admin/summaries/weekly/${id}`, {
        method: 'DELETE',
      });
              ...
              <div className="divide-y">
                {loading ? (
                  ...
                  <div className="p-8 text-center text-gray-500">
                    No summaries found. Click "Create" to generate your first weekly summary.
                  </div>
                ) : (
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500">
                            {summary.article_count} articles • {formatDate(summary.publication_date)}
              ...
              <div className="bg-white rounded-lg shadow p-6 sticky top-4">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Link
                      href={`/weekly/${selectedSummary.publication_date}`}
                      target="_blank"
                      className="text-sm text-blue-600 hover:underline break-all"
                    >
                      /weekly/{selectedSummary.publication_date}
                    </Link>
                  </div>
                  ...
                  {selectedSummary.published_at && (
                    <div>
                      ...
                      <p className="text-sm text-gray-900">
                        {formatDate(selectedSummary.published_at)}
                      </p>
                    </div>
                  )}
